jQuery(document).ready(function() {
    ...
    jQuery('.MuiToolbar-root .MuiBox-root .MuiGrid-root:eq(1) .MuiStack-root:eq(0) .MuiButtonBase-root:eq(0)')
        .prepend('<span class="twbb_editor_nav_menu_structure">Structure:</span>');
    ...
    jQuery(document).on('click','#elementor-editor-wrapper-v2 .MuiGrid-root:nth-child(2) button:eq(0)', function() {
        ...
        setTimeout(function() {
            ...
            let buttonsTemplate = jQuery('#elementor-preview-iframe').contents().find('#twbb_topbar-buttons-from-wn-template').html();
...
function twbb_create_blank_page(that) {
    ...
    const pageData = {
        ...
        content: '',
        status: 'draft',
        slug: jQuery(document).find(".twbb-new-blank-page-input").val(),
    };
...
function twbbSidebarsDeviceModeChanged(mode, sidebar_width) {
    ...
    if( typeof twbb_options.smart_scale_option !== "undefined" && twbb_options.smart_scale_option === 'active' ) {
        ...
        let iframeWidth = windowWidth - sidebar_width;
        let elementor_panel = jQuery("#elementor-panel").width();
        jQuery("#elementor-preview").css({
            "width": iframeWidth + 'px',
            "margin-left": sidebar_width - parseInt(elementor_panel) + 'px',
        });
    }
}
